...
function formatEstimate(row) {
  const lo = row.estimateLow
  const hi = row.estimateHigh
  if (lo == null && hi == null) return '—'
  return formatInrRange(Number(lo ?? 0), Number(hi ?? 0))
}
    ...
    return (
      <div className="flex min-h-[420px] flex-col items-center justify-center rounded-lg border border-brand-brass-pale/40 bg-white px-8 py-16 text-center shadow-sm">
        ...
        <p className="mt-8 max-w-md font-body text-[15px] leading-relaxed text-brand-mist">
          No quote requests yet. Submissions from the Instant Quote flow will appear here.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="font-display text-[26px] font-light text-brand-charcoal lg:text-[28px]">Quote Requests</h1>
          <p className="mt-1 font-body text-[14px] text-brand-mist">Leads from the Instant Quote flow</p>
        </div>
          ...
          <table className="w-full min-w-[1280px] border-collapse text-left">
            <thead>
              <tr className="border-b border-brand-brass-pale/40 bg-brand-ivory-deep/60">
                <th className="whitespace-nowrap px-3 py-3 font-body text-[10px] font-semibold uppercase tracking-[0.08em] text-brand-mist">
                  #
                </th>
                <th className="whitespace-nowrap px-3 py-3 font-body text-[10px] font-semibold uppercase tracking-[0.08em] text-brand-mist">
                  Name
                ...
                <th className="min-w-[140px] px-3 py-3 font-body text-[10px] font-semibold uppercase tracking-[0.08em] text-brand-mist">
                  Scope
                </th>
                <th className="whitespace-nowrap px-3 py-3 font-body text-[10px] font-semibold uppercase tracking-[0.08em] text-brand-mist">
                  Area
                </th>
                <th className="min-w-[100px] px-3 py-3 font-body text-[10px] font-semibold uppercase tracking-[0.08em] text-brand-mist">
                  Location
                </th>
                <th className="whitespace-nowrap px-3 py-3 font-body text-[10px] font-semibold uppercase tracking-[0.08em] text-brand-mist">
                  Estimate Range
              ...
              {pageSlice.map((row, i) => {
                ...
                const mid =
                  row.estimateLow != null && row.estimateHigh != null
                    ? (Number(row.estimateLow) + Number(row.estimateHigh)) / 2
                    : 0
                const emi = mid > 0 ? emiMonthly(mid, 0.12, 36) : 0
                ...
                return (
                  <Fragment key={row.id}>
                    ...
                    >
                      ...
                      <td className="max-w-[140px] truncate px-3 py-3 font-body text-[12px] text-brand-mist" title={displayEmail(row)}>
                        {displayEmail(row)}
                      </td>
                      <td className="max-w-[100px] px-3 py-3 font-body text-[12px] text-brand-charcoal">
                        {row.propertyType != null ? String(row.propertyType) : '—'}
                      </td>
                      <td className="max-w-[180px] px-3 py-3 font-body text-[12px] leading-snug text-brand-charcoal">
                        {formatScope(row)}
                      </td>
                      <td className="whitespace-nowrap px-3 py-3 font-body text-[12px] text-brand-charcoal">{formatAreaSqFt(row)}</td>
                      ...
                      <td className="whitespace-nowrap px-3 py-3 font-body text-[12px] text-brand-charcoal">{formatEstimate(row)}</td>
                          ...
                          <div className="rounded-sm border border-brand-brass-pale/40 bg-white p-6">
                            ...
                            <div className="mt-4 rounded-sm border border-brand-brass-pale/30 bg-brand-ivory-deep/40 p-4">
                              <p className="font-body text-[10px] font-semibold uppercase tracking-[0.1em] text-brand-mist">
                                EMI estimate (indicative)
                              </p>
                              <p className="mt-2 font-body text-[15px] text-brand-charcoal">
                                {mid > 0 ? (
                                  <>
                                    ~{formatInr(emi)} / month at 12% p.a. over 36 months (mid-range project value{' '}
                                    {formatInr(mid)})
                                  </>
                                ) : (
                                  '—'
      ...
      {filtered.length === 0 ? (
        <p className="text-center font-body text-[14px] text-brand-mist">
          No quote requests match your filters. Try adjusting search or date range.
        </p>
      ) : (
